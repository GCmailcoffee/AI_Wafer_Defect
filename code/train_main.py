...
def main(args):
    ...
    # train_dir = os.path.join(data_dir, 'trn')
    # valid_dir = os.path.join(data_dir, 'tst')
    train_set = ImageDataset(train_dir, transform=train_transform)
    valid_set = ImageDataset(valid_dir, transform=valid_transform)

    # 构建DataLoder
    # train_loader = DataLoader(dataset=train_set, batch_size=args.batch_size, shuffle=True, num_workers=args.workers)
    train_loader = DataLoader(dataset=train_set, batch_size=64, shuffle=True, num_workers=args.workers)
